...
export default class Details extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        if (!nextProps.delivery.getOk && this.props.delivery.getOk !== nextProps.delivery.getOk) {
            if (nextProps.delivery.message) {
    ...
    render() {
        ...
        let fileList = delivery.fileList.map(function(val,key){
            return {
                uid: val.fileId,
                name:  val.fileName,
                status: 'done',
                    ...
                    <Card bordered={false} title="">
                        <Row gutter={24}>
                            <Col span={8}>
                                ...
                                <div className={styles.detail}>{delivery.task.taskId}</div>
                            </Col>
                            <Col span={8}>
                                <div className={styles.term}>接收方</div>
                                <div className={styles.detail}>{delivery.task.acceptorTrueName}</div>
                            </Col>
                            <Col span={8}>
                                <div className={styles.term}>时间</div>
                                <div className={styles.detail}>{getDateString(delivery.task.taskCreateTime)}</div>
                            </Col>
                        </Row>
                        <Row gutter={24}>
                            <Col span={8}>
                                <div className={styles.term}>类型</div>
                                <div className={styles.detail}>图片</div>
                            </Col>
                            <Col span={8}>
                                <div className={styles.term}>数量</div>
                                <div className={styles.detail}>{delivery.task.contentCount}</div>
                            </Col>
                            <Col span={8}>
                                <div className={styles.term}>备注</div>
                                <div className={styles.detail}>{delivery.task.taskRemark}</div>
                            </Col>
                        </Row>
                        <Row gutter={24}>
                            <Col span={8}>
                                <div className={styles.term}>当前状态</div>
                                <div className={styles.detail}>{taskStatus[parseInt(delivery.task.taskStatus)]}</div>
                            </Col>
                            <Col span={8}>
                                <div className={styles.term}>版权信息</div>
                                <div className={styles.detail}>{copyrightInfos[parseInt(delivery.task.copyrightInfo)]}</div>
                            </Col>
                        </Row>
                    </Card>
